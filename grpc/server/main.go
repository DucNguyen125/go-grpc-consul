...
func startHtttpServer(port int) {
	...
	if regiErr != nil {
		...
		log.Printf("successfully register service: %s:%v", address, port-1)
	}
...
func main() {
	min := 5000
	max := 8000
	portGrpc := rand.Intn(max-min) + min
	portHttp := portGrpc + 1
	go startHtttpServer(portHttp)
	startGrpcServer(fmt.Sprintf(":%d", portGrpc))
}
